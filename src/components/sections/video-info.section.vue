          ...
          <Section>
               ...
               <div class="sm:flex-row flex-col-reverse flex gap-10" v-if="fileSelected">
                    <div class="opacity-70">
                         ...
                         <ul class="list-disc mt-5 list-inside">
                              <li v-for="(element, i) in infoItems" :key="i">{{ element.label }}</li>
                         </ul>
                    </div>
               ...
               <div class="flex w-full flex-col gap-2" v-else>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                         ...
                         <div class="bg-foreground/5 p-5 border rounded-lg" v-for="(element, i) in cardItems" :key="i">
                              <div class="flex items-top gap-[2px] group">
                                   <div> {{ element.value }} <span class="text-muted-foreground" v-if="element.format"> {{ element.format }} </span> </div>
                              </div>
...
<script setup lang="ts">
     ...
     const cardItems = computed<ICardItem[]>(() => [
          ...
          { label: 'Dimensions', format: 'px', value: `${videoInfo.value.dimensions.width} × ${videoInfo.value.dimensions.height}` },
